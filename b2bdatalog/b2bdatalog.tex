...
can be either additions or retractions, instead of over a full dataset.

Let $C$ be a multiset, referred to as a collection, with $C_t$ being its value at a not necessarily totally
ordered time $t$, and $C_t(b)$ being the possibly negative weight of some record $b \in C_t$. We establish
that the difference of some collection $C$ at time $t$, named $\delta C_t$, is defined as: \[\delta C_t = C_t - C_{t - 1}\],
notably, differences can be both positive, representing additions, or negative, retractions. It therefore
...
collection. Assuming $B$ to be the output of $\mathcal{OP}$ applied over $A$, computations in differential
dataflow follow the following: \[A_t = \sum_{i \leq t} \delta B_i = \mathcal{OP}(\sum_{i \leq t} \delta A_i)\]
...
maintaining $\delta B$ and $\delta $, specifically in the context of iterative dataflows, due to $t$ being
partially ordered.

Let's assume that a datalog program is being evaluated, and four fact updates, labeled as $\delta_0$, $\delta_1$,
$\delta_2$, $\delta_3$, arrive. In regular semi-naive evaluation, even though rule application might happen in parallel,
$\delta_{t + 1}$ will only be evaluated after $\delta_{t}$ is, and the data used to compute each will always consist
of all extensional and intensional, previously inferred, facts.

If program evaluation could be written as a differential dataflow dataflow, there would be two timestamps, $t$
representing the time of arrival of the update, and $\langle a, b \rangle$, with $a$ representing the time $t$ in
which some initial fact was fed into the evaluation, and $b$ denoting the iteration count of the semi-naive
evaluation fixpoint, respecting the following partial order: \[\langle a_i, b_j \rangle \leq \langle c_k, d_l \rangle \iff a_i \leq c_k \wedge b_j \leq d_l\]

If we treat $\alpha_0$, $\alpha_3$, $\alpha_1$ and $\alpha_2$ as differences with the following respective
timestamps: $\langle 0, 1 \rangle, \langle 0, 2 \rangle, \langle 1, 1 \rangle, \langle 1, 2 \rangle$ it is
noticeable that $\alpha_0$ and $\alpha_3$ are comparable, but both of them, and vice-versa, are incomparable
with respect to $\alpha_1$ and $\alpha_2$. This, in turn, has twofold meaning in differential dataflow. First,
that both $\alpha_3$ and $\alpha_2$ could have been inferred in parallel, and secondly, that only their strictly
prior differences were used in their computation.
