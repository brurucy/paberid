...
\begin{abstract}
	...
	all possible data that has been inferred from what is being deleted.
  %   , hence, it is
  % likely that performance is not even across the direction of the update.
  In this	paper we investigate the performance of materialization with
	three reference datalog implementations including a
	lightweight relational engine, and an engine using differential dataflow.
  %
	The output of the paper is coalesced as an extensible reasoning framework, that shares
...
% \textbf{Motivation.}
SQL has been the \textit{de facto} universal relational
...
% Kalmer: no tangents! lets get to the point!
...
% Kalmer: most important? Do not add unimportant claims! 
% The most
\textit{Materialization} is the derivation of all
truths that follow from the application of rules over the relational database, with the goal to ensure queries do not need any reasoning.
%
This, however, is a problem, if the underlying data changes.
%
%
% \textbf{Problem.} 
% The maintenance of materialization is possibly the most important issue
...
therefore there being possibly-severe biases in performance. 
% To this date, there hasn't been
...
% Kalmer: this must be a separete section (it its new), otherwise cut it
% It should be made clear that this is not re-telling of known concepts but the algorithms you propose in Shapiro
\section{Translating Datalog into Relational Algebra}
...
\section{DRED algorithm in Shapiro}
...
\section{Differential Dataflow in Shapiro}
