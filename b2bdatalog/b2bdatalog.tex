...
operators are available in differential dataflow.

\begin{figure} [htb!]
	\centering
	\begin{tikzpicture}[->Stealth,>=stealth',auto,node distance=6cm,
...
Figure \ref{fig:shapiro_comp} depicts the substitution-based method in differential dataflow. Superscripts denote
points of the dataflow that require further explanation. Furthermore, for clarity, we establish the shape of the
data, and the \verb|Var| suffix. We also note that this is a summarized description of the dataflow, with certain
trivial, or too-implementation-oriented, parts having had been omitted.

$\Sigma_0$ is the stream of empty substitutions, which at first will contain only one, indexed per rule identifier. We
assume that rules have an unique identifier. Facts is the relation-indexed stream of facts, and rules is the stream of
rules, notably with two indexes, superscripts $1$ and $2$.
...
\begin{enumerate}
	...
	\item Attempts to create new atoms, that will be used to generate new substitutions
	\item Attempts to create new substitutions
	\item Attempts to create new ground facts
\end{enumerate}
