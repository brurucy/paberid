...
\begin{itemize}
	\item \textbf{Techniques and Guidelines.} We study the challenge of building a reasoner from scratch,
	      ...
	      alongside with relevant recent literature. We also investigate the actual performance cost of supporting
	      common extensions such as non-recursive aggregation, stratified negation, and existential variables.
	...
	\item \textbf{Benchmarking.} We perform two thorough benchmark suites. One evaluates the performance of the developed
	      ...
	      from the program analysis community, heavy users of not-distributed datalog, or from the semantic web community
...
to the immediate consequence, and therefore is the output.

\subsection{Relational Algebra}
Relational Algebra is an imperative language, that explicitly denotes operations over sets of tuples with fixed arity, relations. It is
the most popular database formalism that there is, with virtually every single major database system being a relational database\cite{pg,mysql,sqlserver}
that translate SQL to relational algebra equations.

Due to the extensive amount of shared-memory relational systems, and distributed frameworks

%% Rethink the translation algorithm here, and add an example
%% I'm pretty sure that I could gain a lot of citations just by outlying the algorithm here In a more readable format.
...
\subsubsection{Non-recursive aggregation}

Trivial to implement

\subsubsection{Negation}

a lot of reasoners implement negation, yet curiously do not
benchmark queries that even use it. the cost of stratified
negation is that it prevents the very efficient and obvious
parallelization scheme.

\subsubsection{Existential Variables}

Description logics bla bla bla are too expressive for datalog
however adding skolemized existential variables allows for some
of them to be sort of implemented
