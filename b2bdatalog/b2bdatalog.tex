...
% \textbf{Motivation.}
The canonical language for reasoning over relational databases, oft named as ground fact stores, is the declarative
language datalog\cite{datalog}, whose semantics give meaning to the evaluation of a set of possibly-recursive
restricted horn clauses, namely a program, while remaining not turing complete. To evaluate a program is to
compute implicit consequences of itself over a fact store, providing new facts.

Physically storing all of a program's consequences is called materialization, with the most immediate benefit
being that no reasoning is required during query answering. Maintaining that computation is a indispensable
requirement of modern datalog use-cases, and relates to the more general problem of incremental view maintenance.

Additions are efficiently handled with the semi-naive evaluation method\cite{datalog}, that can be notoriously
efficient for certain classes of programs that are popular in practice. Deletions on the other hand, are possibly
much less efficient, since the retraction of a fact naively implies the deletion of all data derived from it.

The quintessential method for computing the materialization adjustment is delete-rederive\cite{dred}, that generates
new datalog programs to compute the consequences of the deleted ground facts, doing so by first computing all
possible deletions, and then whether there are alternative derivations, with the difference between these sets
being the actual facts that have to be deleted.

The fact that two different algorithms are used implies that there are two different performance characteristics
for additions and deletions, therefore there being possibly-severe biases in performance, such as in the scenario
when either all, or a large portion, of ground facts are deleted, where delete-rederive could be much more expensive
than re-materializing.



A promising computation framework that could be well-suited for incremental materializatin is differential
dataflow\cite{differential_dataflow}.

\paragraph{Contributions.}
To address the aforementioned problem, we conduct a back-to-basics empirical
study of datalog program materialization, revisiting and measuring the core
assumptions that go into the implementation of a reasoner. We coalesce this
knowledge into a rust framework \textbf{Shapiro} that provides multiple shared
components for reasoning research. Straightforward single-threaded, parallel
and distributed implementations, with the last being done with differential
dataflow, of both substitution-based and relational-algebra interpretations are
realized alongside common well-known optimizations.

We perform two thorough benchmark suites. One evaluates the performance of the
developed relational reasoner with multiple different index data structures and
common optimizations, such as string interning and simple parallelism, and
another that attempts to attain the COST\cite{COST} of the reasoner built on
top of differential dataflow. The selected datasets are either from the program
analysis, heavy users of not-distributed datalog, or from the semantic web
community, which has multiple popular infinitely-scalable benchmarks.


\paragraph{Structure of the paper.}

\begin{itemize}
	\item{\textbf{Introduction.}} Differential Dataflow being suitable, and what has been done
	\item{\textbf{Background.}}  Datalog, Infer, DRED
	\item{\textbf{Novelty.}} Differential Dataflow, INFER as a dataflow
	\item{\textbf{System.}} The implementations, the possibly novel? indexing scheme
	\item{\textbf{Benchmarks.}} Differential Dataflow better than everything
...
\subsection{Datalog}
% Kalmer: This subsections should be really short as everybody who reads this paper already knows about datalog. But we cannot delete it completely as then we wont have definitions.
